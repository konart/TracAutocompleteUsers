...
class AutocompleteUsers(Component):
    ...
    def match_request(self, req):
        return '/subjects' in req.path_info
    ...
    def process_request(self, req):
        ...
        if req.args.get('customers'):
            customers = self._get_customers(req)
            #subjects = []
            if customers:
                req.send(json.dumps(customers), 'text/plain')

        if req.args.get('customer_orders'):
            customer_orders = self._get_customer_orders(req)
            #subjects = []
            if customer_orders:
                req.send(json.dumps(customer_orders), 'text/plain')

        if req.args.get('activity'):
            activities = self._get_activities(req)
            subjects = []
            if activities:
                subjects.extend(['%s' % activity for activity in activities])

        if req.args.get('activity_group'):
            activity_groups = self._get_activity_groups(req)
            subjects = []
            if activity_groups:
                subjects.extend(['%s' % activity_group for activity_group in activity_groups])
    ...
    def _get_customers(self, req):
        ...
        db = self.env.get_db_cnx()
        cursor = db.cursor()
        cursor.execute("SELECT name FROM customer where lower(name) like '%"+query+"%'")
        #activities = [user[0] for user in self.env.get_known_users()]
        return sorted([row[0] for row in cursor ])

    def _get_customer_orders(self, req):
        # Returns a list of activities .
        query = req.args.get('term', '').lower()
        customer = req.args.get('customer', '')
        db = self.env.get_db_cnx()
        cursor = db.cursor()
        sql = "SELECT co.number FROM customer_order co left join customer c on c.id=co.customer_id where lower(co.number) like '%"+query+"%' and c.name='"+customer+"'"
        cursor.execute(sql)
        return sorted([row[0] for row in cursor ])

    def _get_activities(self, req):
        # Returns a list of activities .
        db = self.env.get_db_cnx()
        cursor = db.cursor()
        cursor.execute("SELECT name FROM activity")
        #activities = [user[0] for user in self.env.get_known_users()]
        return sorted([row[0] for row in cursor ])

    def _get_activity_groups(self, req):
        # Returns a list of activities .
        #query = req.args.get('q', '').lower()
        activity = req.args.get('activity', '')
        db = self.env.get_db_cnx()
        cursor = db.cursor()
        cursor.execute("SELECT ag.name FROM activity_group ag left join activity a on a.id=ag.activity_id where a.name='"+activity+"'")
        return sorted([row[0] for row in cursor ])
